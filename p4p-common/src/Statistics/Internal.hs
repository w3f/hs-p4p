...
defaultShow1 con a n =
  showParen (n >= 11) (showString con . showChar ' ' . showsPrec 11 a)

defaultShow2 :: (Show a, Show b) => String -> a -> b -> Int -> ShowS
defaultShow2 con a b n = showParen
  (n >= 11)
  ( showString con
  . showChar ' '
  . showsPrec 11 a
...
defaultShow3
  :: (Show a, Show b, Show c) => String -> a -> b -> c -> Int -> ShowS
defaultShow3 con a b c n = showParen
  (n >= 11)
  ( showString con
  . showChar ' '
  . showsPrec 11 a
...
defaultReadPrecM2
  :: (Read a, Read b) => String -> (a -> b -> Maybe r) -> ReadPrec r
