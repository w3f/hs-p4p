...
instance Read WeibullDistribution where
  readPrec =
    defaultReadPrecM2 "weibullDistr"
...
instance D.FromSample WeibullDistribution Double where
  fromSample xs
    ...
    where (m, v) = S.meanVarianceUnb xs
...
quantile (WD k l) p
  ...
  | otherwise
  = error
    $ "Statistics.Distribution.Weibull.quantile: p must be in [0,1] range. Got: "
    ++ show p
  where inf = 1 / 0
...
complQuantile (WD k l) q
  ...
  | otherwise
  = error
    $ "Statistics.Distribution.Weibull.complQuantile: q must be in [0,1] range. Got: "
    ++ show q
  where inf = 1 / 0
