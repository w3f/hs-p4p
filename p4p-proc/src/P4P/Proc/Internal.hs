...
-- | Convenience / compatibility alias for 'PEnt' in the contex of a 'Proc'.
type PAddr ps = PEnt ps

-- | Input from the runtime execution environment to a process.
--
-- The 'task' type-param is provided for re-using this data structure for other
-- purposes; both p4p-rt and p4p-sim instantiates it to '()'.
...
data RuntimeO addr = RTAddr !(Observations addr)
  deriving (Eq, Ord, Show, Read, Generic, Binary, Serialise)

type GMsgI ps = GMsg (RuntimeI ()) (UserI ps) (PMsg ps) Void
type GMsgO ps = GMsg (RuntimeO (Ent (PMsg ps))) (UserO ps) (PMsg ps) (AuxO ps)
